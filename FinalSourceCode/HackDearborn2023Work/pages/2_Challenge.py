...
data = st.file_uploader("xlsx file", type =['xlsx'] )
...
if data is not None:
    ...
    x = st.text_area("Enter question about data:")

    if st.button("Submit Data Question!"):
        if x:
            with st.spinner("Awaiting Response..."):
              llm = ChatOpenAI(model_name='gpt-4', temperature=0.0)
              agent = create_pandas_dataframe_agent(llm, df, verbose=True)
              st.write(agent.run(x))
        else:
...
def main():
    ...
    prompt = st.text_area("Enter question about heatmap!:")

    if st.button("Submit Heatmap Question"):
        if prompt:
